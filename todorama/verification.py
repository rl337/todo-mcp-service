...
class VerificationSystem:
    ...
    def verify_work(
        ...
        if not (self.project_root / "src" / "main.py").exists():
            return {
                ...
                "passed": False,
                "message": f"Not in project root. Expected todorama/main.py in {self.project_root}"
            }
    ...
    def _check_code_quality(self) -> VerificationResult:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["python3", "-c", "from todorama.database import TodoDatabase; from todorama.main import app"],
                    cwd=self.project_root,
                    capture_output=True,
                    text=True,
                    timeout=10
